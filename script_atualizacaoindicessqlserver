-- Inicio script
--drop PROCEDURE [dbo].[stpAtualiza_Indices]
...
BEGIN
	...
	-- Declara variaveis (métricas) previamente definidas para os limiares de fragmentação e ScanDensity
	DECLARE @maxLogicalFrag TINYINT,
	@minScanDensity TINYINT

	-- Seta variaveis (métricas) previamente definidas para os limiares de fragmentação e ScanDensity
	...
	;WITH Tamanho_Tabelas AS (
		...
		FROM #showcontig
		WHERE (ScanDensity = @maxLogicalFrag)
		  AND (ScanDensity = 50)
	)
